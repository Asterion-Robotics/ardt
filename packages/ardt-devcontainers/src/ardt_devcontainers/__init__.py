...
and the editor wiring live in ardt as package data and update with the pinned
ardt version, exactly as the CI image recipe does in ``ardt-ros-pipelines``.
...
This distribution is the **engine**, and it is domain-neutral: it knows how to
render, hash, provision and drive, and nothing about what kind of repo it is
rendering for. That knowledge is a :class:`~ardt_devcontainers.profiles.Profile`
contributed through the ``ardt.dev_profiles`` entry-point group — ``ardt-ros-dev``
ships the ``ros2`` one. The engine imports no profile plugin.
...
__version__ = installed("ardt-devcontainers")
