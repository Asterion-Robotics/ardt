...
An image-producing pipeline has two artifact destinations, each an explicit flag: `--publish` pushes the multi-arch image to the registry (and errors when none is configured), `--load` puts the native-arch image into the local docker daemon as `<project>:<version>` for `docker run`-level poking. The exported `Dockerfile.rendered` stays the third, ardt-free route: `docker build -f pipeline-reports/Dockerfile.rendered .` reproduces the image with nothing installed.
